...
public class ObstacleFactory {
    ...
    public void releaseAllObstacles() {
        for (WeightedCreator wc : weightedCreators) wc.creator.releaseAll();
    }

    public List<BaseObstacle> getAllInUseObstacles() {
        List<BaseObstacle> list = new ArrayList<>();
        for (WeightedCreator wc : weightedCreators) list.addAll(wc.creator.getInUse());
        return list;
    }

    public List<String> getRegisteredCreatorNames() {
        List<String> names = new ArrayList<>();
        for (WeightedCreator wc : weightedCreators) names.add(wc.creator.getName());
        return names;
    }
}
